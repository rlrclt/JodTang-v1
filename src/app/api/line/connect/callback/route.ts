...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!channelId || !channelSecret) {
      return NextResponse.redirect(`${settingsUrl}?line_error=missing_line_config`);
    }

    if (!supabaseUrl || !supabaseKey) {
      console.error("Missing Supabase config:", { url: !!supabaseUrl, key: !!supabaseKey });
      return NextResponse.redirect(`${settingsUrl}?line_error=missing_supabase_config`);
    }
    ...
    // 2. ดึง LINE Profile ด้วย access_token
    ...
    // 4. ส่งข้อความยืนยันผ่าน LINE Bot
    ...
    // 5. redirect กลับหน้า settings/profile
    return NextResponse.redirect(`${settingsUrl}?line_connected=1`);
  } catch (err) {
    console.error("LINE connect callback unhandled error:", err);
    return NextResponse.redirect(`${settingsUrl}?line_error=internal`);
  }
}
