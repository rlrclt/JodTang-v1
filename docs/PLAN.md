...
จดบันทึกรายรับรายจ่าย ใช้เองและให้เพื่อนใช้ · จุดที่เน้นเป็นพิเศษ: **AI ช่วยวิเคราะห์ว่ารายจ่ายหมดไปกับอะไร**

โปรเจกต์นี้ **เขียนใหม่ทั้งหมด** — ไม่ยกโค้ด/ดีไซน์/สคีมาจากโปรเจกต์ก่อนหน้ามาใช้ (กฎอยู่ใน `.hermes.md`)
สถานะ: **สเปกฉบับร่าง รอคุณอนุมัติ** — ยังไม่เริ่มเขียนโค้ด (การ์ด scaffold รันคู่ขนานได้เพราะไม่พึ่งคำตอบที่ค้าง)
ไฟล์นี้คือแหล่งความจริงของขอบเขตงาน · คำถามที่ยังไม่ตัดสินอยู่ที่ §9
...
## 11. เครื่องมือ/infra ที่ตัดสินแล้ว

**Supabase MCP — ใช้แบบ project-scoped เต็ม** (ยืนยันโดย yoru 2026-09-16) เพื่อให้ `apply_migration` / `execute_sql` ทำงานได้จากงานของทีม
- ตั้งเมื่อ **มีโปรเจกต์ dev แล้วเท่านั้น** · สโคปที่โปรเจกต์ dev เท่านั้น — **ห้ามชี้โปรเจกต์ production เด็ดขาด** (ห้ามใช้ MCP กับ prod แม้ในอนาคต; งาน prod ให้ใช้ CLI/คนกด)
